...
export const TransactionTable: React.FC<TransactionTableProps> = ({ 
  transactions, 
  openingBalance,
  accountName = "Cash Account" // Default title if not provided
}) => {
  ...
  const generatePDF = () => {
    ...
    doc.save(`transactions-${safeAccountName}-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
  };
  ...
  const generateExcel = () => {
    ...
    const safeAccountName = accountName.replace(/[^a-z0-9]/gi, '-').toLowerCase();
    const fileName = `transactions-${safeAccountName}-${today}.xlsx`;

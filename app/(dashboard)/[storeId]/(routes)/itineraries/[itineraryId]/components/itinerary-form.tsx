...
export const ItineraryForm: React.FC<ItineraryFormProps> = ({
  ...
  const onSubmit = async (data: ItineraryFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/itineraries/${params.itineraryId}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/itineraries`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error('Make sure you removed all Itinerary using this billboard first.');
    } finally {
      setLoading(false);
      setOpen(false);
    }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input disabled={loading} placeholder="Itinerary label" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}

...
import { Location, Image, Hotel, TourPackageQuery, Itinerary } from "@prisma/client"
...
export const TourPackageQueryForm: React.FC<TourPackageQueryFormProps> = ({
  ...
  const onSubmit = async (data: TourPackageQueryFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/tourPackageQuery/${params.tourPackageQueryId}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/tourPackageQuery`, data);
      }
      router.refresh();

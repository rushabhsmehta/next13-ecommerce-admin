  ...
  return (
    <> 
      <div className="flex items-center justify-between">
        <Heading title={`Tour Package Query (${data.length})`} description="Manage tour package Query for your store" />
        <Button onClick={() => router.push(`/${params.storeId}/tourPackageQuery/new`)}>
          <Plus className="mr-2 h-4 w-4" /> Add New
        </Button>
      </div>
      <Separator />
      <DataTableMultiple searchKeys={["tourPackageQueryNumber", "customerNumber", "customerName", "tourPackageQueryName", "assignedTo"]} columns={columns} data={data} />

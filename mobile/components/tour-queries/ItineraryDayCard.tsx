...
export function ItineraryDayCard({
  ...
  const plainNotes = extractPlainText(it.notes);
  ...
  return (
    <View style={styles.dayCard} testID={`tq-edit-day-${idx}`}>
      ...
      <Pressable
        accessibilityRole="button"
        accessibilityLabel={notesOpen ? "Hide notes editor" : "Show notes editor"}
        style={styles.inlineToggle}
        onPress={() => setNotesOpen((o) => !o)}
        testID={`tq-edit-day-notes-toggle-${idx}`}
      >
        <Text style={styles.inlineToggleText}>
          Notes {notesOpen ? "· hide" : "· expand"} (optional)
        </Text>
        <Ionicons
      ...
      {notesOpen ? (
        <TextInput
          style={[styles.input, styles.textarea]}
          value={plainNotes}
          onChangeText={(t) => onChange({ ...it, notes: t })}
          multiline
          placeholder="Special notes for this day..."
          placeholderTextColor={Colors.textTertiary}
          testID={`tq-edit-day-notes-${idx}`}
        />
      ) : null}
      <FormField

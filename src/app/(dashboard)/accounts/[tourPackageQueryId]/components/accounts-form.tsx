  ...
  const transformInitialData = (data: any) => {
    return {
      ...
      purchaseDetails: data.purchaseDetails?.map((purchaseDetail: any) => ({
        ...
        gstAmount: purchaseDetail.gstAmount || 0,
        gstPercentage: purchaseDetail.gstPercentage || 0,
      ...
      saleDetails: data.saleDetails?.map((saleDetail: any) => ({
        ...
        gstAmount: saleDetail.gstAmount || 0,
        gstPercentage: saleDetail.gstPercentage || 0,
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          <Tabs defaultValue="purchaseDetails">
            ...
            <TabsContent value="purchaseDetails">
              {purchaseFields.map((field, index) => (
                <div key={field.id} className="space-y-2 border p-2 mb-2 rounded">
                        ...
                        <FormControl>
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              // Auto-calculate GST if percentage is set
                              const percentage = form.getValues(`purchaseDetails.${index}.gstPercentage`) || 0;

                              if (percentage > 0) {
                                const gstAmount = calculateGSTAmount(value, percentage);
                                form.setValue(`purchaseDetails.${index}.gstAmount`, gstAmount);
                              }
                            }}
                            value={field.value}
                          />
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              if (price > 0 && percentage > 0) {
                                const gstAmount = calculateGSTAmount(price, percentage);
                                form.setValue(`purchaseDetails.${index}.gstAmount`, gstAmount);
                              }
                            }}
                            value={field.value || 0}
                  ...
                  <FormField
                    control={form.control}
                    name={`purchaseDetails.${index}.gstAmount`}
                    render={({ field }) => (
                      <FormItem>
            ...
            <TabsContent value="saleDetails">
                        ...
                        <FormControl>
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              if (percentage > 0) {
                                const gstAmount = calculateGSTAmount(value, percentage);
                                form.setValue(`saleDetails.${index}.gstAmount`, gstAmount);
                              }
                            }}
                            value={field.value}
                          />
              ...
              <Button type="button" onClick={() => appendSale({
                customerId: '',
                saleDate: new Date(),
                salePrice: 0,
                gstAmount: 0,

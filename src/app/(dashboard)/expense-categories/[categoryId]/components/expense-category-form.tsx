...
type ExpenseCategoryFormValues = z.infer<typeof formSchema>
...
export const ExpenseCategoryForm: React.FC<ExpenseCategoryFormProps> = ({
  ...
  const description = initialData ? 'Edit an expense category.' : 'Add a new expense category';
  const toastMessage = initialData ? 'Expense category updated.' : 'Expense category created.';
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/expense-categories/${params?.categoryId}`);
      router.refresh();
      router.push(`/expense-categories`);

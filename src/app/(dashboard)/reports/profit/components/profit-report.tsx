...
import { GSTSection } from './sections/gst-section';
...
export default function ProfitReport({ initialData, generalExpenses, generalIncomes }: ProfitReportProps) {
    ...
    // Extract purchases and sales from initialData for GST reporting
    const allPurchases = useMemo(() => {
        return initialData.flatMap(item => item.purchaseDetails || []);
    }, [initialData]);

    const allSales = useMemo(() => {
        return initialData.flatMap(item => item.saleDetails || []);
    }, [initialData]);

    // Filter purchases and sales based on date range
    const filteredPurchases = useMemo(() => {
        return allPurchases.filter(item => {
            const purchaseDate = new Date(item.purchaseDate);
            return (!date?.from || isAfter(purchaseDate, date.from)) &&
                (!date?.to || isBefore(purchaseDate, date.to));
        });
    }, [allPurchases, date]);
    ...
    const filteredSales = useMemo(() => {
        return allSales.filter(item => {
            ...
            return (!date?.from || isAfter(saleDate, date.from)) &&
                (!date?.to || isBefore(saleDate, date.to));
        });
    }, [allSales, date]);

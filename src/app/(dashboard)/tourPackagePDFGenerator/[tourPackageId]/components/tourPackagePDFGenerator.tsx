...
const TourPackagePDFGenerator: React.FC<TourPackagePDFGeneratorProps> = ({
  ...
  const buildFooterHtml = useCallback((): string => {
    ...
    const social = {
      ...
      twitter: safeValue(company.social?.twitter),
    };
  ...
  const buildHtmlContent = useCallback((): string => {
    ...
    const hotelsSection =
      hotelsLinkedToItinerary.length > 0
        ? `
                    ...
                    ? `<div style="${avoidPageBreakStyle}">
                        ...
                        <div style="margin-top: 8px; display: flex; flex-direction: column; gap: 8px;">
                          ${Object.entries(companyProfile.social)
                            .filter(([, url]) => !!url)
                            .map(
                              ([platform, url]) => `<a href="${sanitizeText(url)}" style="font-size: 13px; color: ${brandColors.secondary}; text-decoration: none; font-weight: 600; display: inline-flex; gap: 8px; align-items: center;">
                                  <span style="width: 26px; height: 26px; border-radius: 50%; background: ${brandColors.lightOrange}; display: inline-flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700;">${platform.slice(0, 2).toUpperCase()}</span>
                                  <span style="letter-spacing: 0.4px;">${platform}</span>
                                </a>`
                            )
                            .join("")}
                        </div>

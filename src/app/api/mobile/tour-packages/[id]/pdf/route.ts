...
/**
 * Server-renders the tour-package PDF (with optional variants block) and
 * streams it back to mobile, mirroring the tour-query PDF route. Re-uses the
 * existing `tourPackagePDFGenerator` / `tourPackagePDFGeneratorWithVariants`
 * pages via Puppeteer so HTML composition stays in one place.
 *
 * `?variant=1` switches to the with-variants generator page.
 */
export async function GET(req: Request, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    await recordMobileAudit({
      ...
      entityId: id,
      action: "READ",
      metadata: { withVariants, bytes: pdf.length },
    });

    const safeName =
      (pkg.tourPackageName || id)
        .toString()
        .replace(/[^a-zA-Z0-9_-]+/g, "-")
        .slice(0, 60) || "tour-package";
    const suffix = withVariants ? "-variants" : "";

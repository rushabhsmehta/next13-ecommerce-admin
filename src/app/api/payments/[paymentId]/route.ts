...
export async function GET(req: Request, props: { params: Promise<{ paymentId: string }> }) {
...
export async function DELETE(req: Request, props: { params: Promise<{ paymentId: string }> }) {
  const params = await props.params;
  return handleApi(async () => {
    const { userId } = await auth();
    if (!userId) return jsonError('Unauthenticated', 403, 'AUTH');

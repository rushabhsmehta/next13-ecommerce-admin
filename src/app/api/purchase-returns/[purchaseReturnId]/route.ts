...
export async function GET(req: Request, props: { params: Promise<{ purchaseReturnId: string }> }) {
...
export async function PATCH(req: Request, props: { params: Promise<{ purchaseReturnId: string }> }) {
  const params = await props.params;
  try {
    const { userId } = await auth();
...
export async function DELETE(req: Request, props: { params: Promise<{ purchaseReturnId: string }> }) {
  const params = await props.params;
  try {
    const { userId } = await auth();
    if (!userId) {

...
export async function PATCH(req: Request, props: { params: Promise<{ purchaseId: string }> }) {
  const params = await props.params;
  try {
    const { userId } = await auth();
    if (!userId) {
...
export async function DELETE(req: Request, props: { params: Promise<{ purchaseId: string }> }) {
  const params = await props.params;
  try {
    const { userId } = await auth();
    if (!userId) {

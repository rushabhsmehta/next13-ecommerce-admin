...
export async function GET(req: Request, props: { params: Promise<{ saleReturnId: string }> }) {
...
export async function PATCH(req: Request, props: { params: Promise<{ saleReturnId: string }> }) {
  const params = await props.params;
  try {
    const { userId } = await auth();
...
export async function DELETE(req: Request, props: { params: Promise<{ saleReturnId: string }> }) {
  const params = await props.params;
  try {
    const { userId } = await auth();
    if (!userId) {
